...
import Dict
import Domain.Store.Utils exposing (getItemPps)
import Domain.Store.Item exposing (ItemId(..), itemMeta)
        ...
        [ test "Grandma self-boost" <|
            \_ ->
                let
                    ...
                    -- We expect (1.0 + 0.0) * (1.02^5-1) * (1.005^5) * 1.0
        ...
        , test "Cursor gets flat boost from Factories: getCursorFactoryBonus" <|
            \_ ->
                let
                    ...
                    -- expect 0.01 * 26, since 26 other items
                    expected =  0.01 * 26
        ...
        , test "Neighbor bonus" <|
            ...
            \_ ->
                let
                    ...
                    expected =  (0.1 + 0.01*26) * (1.02^0) * (1.005^25) * 1.0
                    result = getItemPps item owned
                in
                result |> Expect.within (Expect.Absolute 0.0001) expected
